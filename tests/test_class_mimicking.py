...
def check_members_equality(
    obj1, obj2, verbose: bool = False, omitted_attributes: list = None
):
...
def check_dict_members(
    initial_dict, included_keys, additional_keys, omitted_attributes=None
):
    new_dict = deepcopy(initial_dict)
    check_members_equality(
        initial_dict, new_dict, omitted_attributes=omitted_attributes
    )
...
def check_list_members(
    initial_list, included_objects, additional_objects, omitted_attributes=None
):
    new_list = deepcopy(initial_list)
    check_members_equality(
        initial_list, new_list, omitted_attributes=omitted_attributes
    )
...
class TestClassMimicking(TestCase):
    ...
    def test_profile_members(self):
        ...
        check_list_members(
            ...
            self.BALLOTS,
            self.EXTRA_BALLOTS,
            omitted_attributes=["_ballots_list", "_ballot_container"],
        )
    ...
    def test_multiprofile_members(self):
        ...
        check_dict_members(
            multiprofile,
            [b.freeze() for b in self.BALLOTS],
            [b.freeze() for b in self.EXTRA_BALLOTS],
            omitted_attributes=["_ballots_counter", "_ballot_container"],
        )
